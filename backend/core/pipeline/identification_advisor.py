...
verdict 选择标准（**只有两种**：accept / downgrade，没有 reject）：
- accept：模型 R²>0.6、K 符号合理、T 在回路类型量级范围内、无明显数据问题，可直接整定
- downgrade：任何不达标情形都归到这里 —— 既包括轻微可疑（R² 0.3-0.6 / T 偏小 / 数据噪声死区），
  也包括严重问题（K 符号错 / R²<0.3 / T 接近 dt / 多个 attempts 失败 / corr 极低）。
  系统不会因此中止，而是会带着"不可信"标记继续整定，让用户拿到尽力而为的结果 + 明确警告。
  对于严重问题的 downgrade，请在 concerns 里把每条问题写清楚，便于下一轮"精修辨识"按你的判断去重试。
    ...
    *,
    ...
    if not model_cfg.model_api_key or not model_cfg.model_api_url:
        ...
        return _failure_result(
            ...
            error_message="MODEL_API_KEY / MODEL_API_URL 未配置",
        )
    ...
    try:
        ...
        return _failure_result(
            ...
            error_message=str(exc)[:300] or "LLM 调用失败",
        )
    ...
    # Phase 1: reject 已废弃；如果旧 prompt 缓存或 LLM 仍输出 reject，归并为 downgrade
    if verdict == "reject":
        verdict = "downgrade"
    if verdict not in ("accept", "downgrade"):
        logger.warning("模型评审返回非法 verdict：%r", verdict)
        return _failure_result(
            error_type="invalid_verdict",
            error_message=f"模型评审返回非法 verdict: {verdict}",
            raw_text=raw_text,
        )
